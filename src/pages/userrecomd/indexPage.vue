<template>
  <div style="padding:0 5px;">
    ...
    <div class="centerView">
      ...
      <div class="innerView">
        <div><i-icon  class="micon" type="createtask" size="28" color="red" @click="toSonglistPage"/></div>
      ...
      <div class="gdView">
        <div class="gdInnerView" v-for="item in recommendResource" :key="item.id" :id="item.id" @click='toPlayListDetail(item.id)'>
          <div class="playCountMain">
            <div class="playCount"><i-icon type="customerservice" /><span>{{item.playCount}}</span></div>
    ...
    created(){
      ...
      this.$nextTick(()=>
        ...
        fly.all([getMvFist(),getNewSong(),getRecomRes(),getPersDj()]).then(fly.spread((mvF,NewS,RecomRes,PresDj)=>{
          ...
          that.recommendResource = RecomRes.data.result.slice(0, 6).map(item=>{item.playCount=parseInt(item.playCount/10000) + '万';return item;});
